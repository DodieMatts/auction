...
import { getBidderAuctionResults } from "@/lib/bidder/bidder-auctions-api";
...
export async function GET(
  ...
  try {
    return jsonResponse(await getBidderAuctionResults(auctionId), 200);
  } catch (error) {
...
function safeMessage(status: number, message: string): string {
  ...
  if (status === 409) return "Auction results are not available";
